...
- 🤖 **자동화** — 구글 폼을 통해 제출 시 자동으로 데이터베이스에 등록
...
│
├── api/                           # API 엔드포인트 분리
│   ├── numbers.php                # 합격/불합격/지원불가자 확인
│   ├── unique_numbers.php         # 데이터베이스에 저장된 고유번호
│   ├── phpinfo.php                # 실행 서버의 정보
│   └── check.php                  # 클라이언트가 확인
│
├── assets/
│   ├── css/                       # 스타일시트
...
│
├── index.php                      # /api/check.php 로 이동 (클라이언트)
├── login.php                      # /admin/login.php 로 이동 (관리자)
├── .gitignore                     # Git 무시 파일
└── README.MD                      # 이 파일
```
